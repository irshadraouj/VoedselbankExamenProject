...
class HelperService
{
    ...
    public static function action($method, $uri= '/') {
        $cacheKey = 'dunique_'.$method.'action_url';
        // Check wether the action url is already set
        if (isset(ee()->session) && ee()->session->cache($cacheKey, $method)){
            $action_url = ee()->session->cache($cacheKey, $method);
        } else {
            ...
            if (isset(ee()->session)) {
                ee()->session->set_cache($cacheKey, $method, $action_url);
            }
        }
        return $action_url;
    }
